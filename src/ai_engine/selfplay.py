...
class SelfPlay:
    """Generates training data by having the model play against itself."""
    ...
    def play_game(self, temperature=1.0, temp_threshold=10):
        board = chess.Board()
        history = []  # list of (state, policy) tuples

        for move_num in range(self.max_moves):
            state = board_to_tensor(board)[0]  # drop batch dim for storage
            ...
            # sample move during exploration, pick best during exploitation
            if temp > 0.5:
    ...
    def _label_outcomes(self, history, result):
        """Assign +1/-1/0 value to each position based on who won."""
        if result == "1-0":
            outcome = 1.0
        elif result == "0-1":
            outcome = -1.0
        ...
        for state, policy in history:
            # check turn channel to see who was playing
            was_white = state[0, 0, 12] == 1.0
            val = outcome if was_white else -outcome
            states.append(state)
    ...
    def generate_games(self, n_games, temperature=1.0):
        """Play n_games and concatenate all training data."""
        ...
        for i in range(n_games):
            ...
            print(f"  game {i+1}/{n_games} done ({result})")
...
class Arena:
    """Pit two models against each other to see which is stronger."""
    ...
    def play_game(self, model1_white=True):
        ...
        for _ in range(self.mcts1.n_sims):  # TODO: should probably use max_moves here
    ...
    def evaluate(self, n_games=10):
        wins = 0
        for i in range(n_games):
            # alternate colors so it's fair
            white = (i % 2 == 0)
            wins += self.play_game(white)
        return wins / n_games
